...
public class ArticleCreatedTimeRepository {
    /*
        이거는 왜 저장해야하는 지 의아해 할 수 있다.
        우리 방식에서는 인기글 선정하는 게 오늘 생성된 게시글만 해야 된다.

        좋아요 이벤트가 왔는데, 이 이벤트에 대해서 게시글이 오늘 게시글인지 확인하려면, 게시글 서비스에 조회가 필요함.
        하지만 자체적으로 우리가 게시글 생성 시간을 저장하고 있으면 이게 오늘 게시글인지 게시글 서비스에게 물어보지 않아도 바로 알 수 있다.
        그래서 이렇게 생성시간도 우리가 관리를 해 주도록 하겠다.
     */
